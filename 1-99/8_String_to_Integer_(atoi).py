...
#   2 Check if the next character (if not already at the end of the string) is '-' or '+'. Read this character in if it is either. This determines
...
class Solution:
    def myAtoi(self, input: str) -> int:
        ...
        if idx < len(input) and input[idx] == "+":
            sign = 1
            idx += 1
        elif idx < len(input) and input[idx] == "-":
            sign = -1
            idx += 1

        while idx < len(input) and input[idx].isdigit():
            digit = int(input[idx])
