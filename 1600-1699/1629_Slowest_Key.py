...
# 0-indexed. The 0th key was pressed at the time 0, and every subsequent key was pressed at the exact time the previous
# key was released.
#
...
class Solution:
    def slowestKey(self, releaseTimes: List[int], keysPressed: str) -> str:
        ...
        for t, ch in zip(releaseTimes, keysPressed):
            dt = t - t0
            if dt == longestduration and ch > key:
                key = ch
            elif dt > longestduration:
                key = ch
                longestduration = dt
            t0 = t
