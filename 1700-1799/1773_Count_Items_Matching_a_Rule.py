...
# ruleValue = "phone"
# Output: 2
# Explanation: There are only two items matching the given rule, which are ["phone","blue","pixel"] and ["phone","gold",
# "iphone"]. Note that the item ["computer","silver","phone"] does not match.
#
#
# Constraints:
# 1 <= items.length <= 10^4
...
class Solution:
    def countMatches(self, items: List[List[str]], ruleKey: str, ruleValue: str) -> int:
        ...
        for item in items:
            if item[{"type": 0, "color": 1, "name": 2}[ruleKey]] == ruleValue:
                c += 1
