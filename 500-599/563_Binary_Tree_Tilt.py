...
# Tilt of node 1 : |2-3| = 1 (left subtree is just left child, so sum is 2; right subtree is just right child, so sum is
# 3)
...
# Tilt of node 2 : |3-5| = 2 (left subtree is just left child, so sum is 3; right subtree is just right child, so sum is
# 5)
...
class Solution:
    def findTilt(self, root: TreeNode) -> int:
        def helper(root: TreeNode) -> Tuple[int, int]:
            if root:
                tilt_l, s_l = helper(root.left)
                tilt_r, s_r = helper(root.right)
                return abs(s_l - s_r) + tilt_l + tilt_r, s_l + s_r + root.val
            else:
                return 0, 0
