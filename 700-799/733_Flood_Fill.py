...
class Solution:
    def floodFill(
        ...
        while stack:
            ...
            if r > 0 and image[r - 1][c] == initColor:
                stack.append((r - 1, c))
            if r < len(image) - 1 and image[r + 1][c] == initColor:
                stack.append((r + 1, c))
            if c > 0 and image[r][c - 1] == initColor:
                stack.append((r, c - 1))
            if c < len(image[0]) - 1 and image[r][c + 1] == initColor:
                stack.append((r, c + 1))
